...
class detectormanos:
    ...
    def find_position(self, frame, hand = 0, draw_points: bool = True, draw_box: bool = True):
        xlist = []
        ylist = []
        bbox = []
        self.hands_list = []
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(my_hand.landmark):
                ...
                xlist.append(cx)
                ylist.append(cy)
                self.hands_list.append([id, cx, cy])
                if draw_points:
        ...
        return self.hands_list, bbox

    def fingers_up(self):
        fingers = []
        if self.hands_list[self.tip[0]][1] > self.hands_list[self.tip[0] - 1][1]:
            fingers.append(1)
        else:
            fingers.append(0)

        for i in range(1, 5):
            if self.hands_list[self.tip[i]][2] < self.hands_list[self.tip[i] - 2][2]:
                fingers.append(1)
            else:
                fingers.append(0)
    ...
    def distance(self, p1, p2, frame, draw: bool = True, radio: int = 15, thickness: int = 3):
